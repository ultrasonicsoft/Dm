...
using Ultrasonic.DownloadManager.Core.Internal;
...
namespace Ultrasonic.DownloadManager.Controls
{
    ...
    public class AutoGrid : Grid
    {
        ...
        /// <summary>
        /// Gets or sets a value indicating whether the grid's generated row/column definitions use <see cref="GridLength.Auto"/> height/width instead of the default (*).
        /// </summary>
        /// <value><c>true</c> if the grid uses auto sized row/column definitions; otherwise, <c>false</c>.</value>
        public bool UseAutoSize
        {
            get { return (bool)GetValue(UseAutoSizeProperty); }
            set { SetValue(UseAutoSizeProperty, value); }
        }

        /// <summary>
        /// Identifies the <see cref="UseAutoSize"/> dependency property.
        /// </summary>
        public static readonly DependencyProperty UseAutoSizeProperty =
            DependencyProperty.Register("UseAutoSize", typeof(bool), typeof(AutoGrid), new FrameworkPropertyMetadata(false, OnPropertyChanged));
                ...
                (!isVertical && _rowOrColumnCount != RowDefinitions.Count))))
            {
                ...
                if (IsAutoIndexing)
                {
                    ...
                    if (isVertical)
                    {
                        ...
                        if (useAutoSize)
                        {
                            foreach (RowDefinition definition in RowDefinitions)
                            {
                        ...
                        else
                        {
                            foreach (RowDefinition definition in RowDefinitions)
                            {
                                ClearValue(RowDefinition.HeightProperty);
                            }
                        }
                    }
                    else // horizontal
                    {
                        ...
                        else
                        {
                            foreach (ColumnDefinition definition in ColumnDefinitions)
                            {
                                ClearValue(ColumnDefinition.WidthProperty);
                            }
                        }
                    }
                }
                ...
                foreach (UIElement child in Children)
                {
                    ...
                    if (ChildMargin != null)
                    {
                        DependencyHelper.SetIfNonLocal(child, FrameworkElement.MarginProperty, ChildMargin.Value);
                    }
                    if (ChildHorizontalAlignment != null)
                    {
                        DependencyHelper.SetIfNonLocal(child, FrameworkElement.HorizontalAlignmentProperty, ChildHorizontalAlignment.Value);
                    }
                    if (ChildVerticalAlignment != null)
                    {
                        DependencyHelper.SetIfNonLocal(child, FrameworkElement.VerticalAlignmentProperty, ChildVerticalAlignment.Value);
                    }
                }
            }
