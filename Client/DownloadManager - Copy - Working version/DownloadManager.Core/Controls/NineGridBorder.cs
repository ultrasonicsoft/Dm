        ...
        private void DrawImage(DrawingContext dc, Rect rect)
        {
            ...
            if (source != null)
            {
                ...
                if (IsNineGrid)
                {
                    ...
                    double[] xGuidelines = new double[] { 0, margin.Left, rect.Width - margin.Right, rect.Width };
                    double[] yGuidelines = new double[] { 0, margin.Top, rect.Height - margin.Bottom, rect.Height };
                    GuidelineSet guidelineSet = new GuidelineSet(xGuidelines, yGuidelines);
                    guidelineSet.Freeze();

                    dc.PushGuidelineSet(guidelineSet);

                    double[] vx = new double[] { 0D, margin.Left / source.Width,
                        (source.Width - margin.Right) / source.Width, 1D };
                    double[] vy = new double[] { 0D, margin.Top / source.Height,
                        (source.Height - margin.Bottom) / source.Height, 1D };
                    double[] x = new double[] { rect.Left, rect.Left + margin.Left, rect.Right - margin.Right, rect.Right };
                    double[] y = new double[] { rect.Top, rect.Top + margin.Top, rect.Bottom - margin.Bottom, rect.Bottom };
